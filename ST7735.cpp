...
//------------ST7735_AddColors------------
// (Matthew)
uint16_t ST7735_AddColors(uint16_t c1, uint16_t c2) {
	uint16_t r = (c1 & 0b11111) + (c2 & 0b11111);
	if(r > 0b11111) r = 0b11111;
	uint16_t g = (c1 & 0b11111100000) + (c2 & 0b11111100000);
	if(g > 0b11111100000) g = 0b11111100000;
	uint32_t b = (c1 & 0b1111100000000000) + (c2 & 0b1111100000000000);
	if(b > 0b1111100000000000) b = 0b1111100000000000;
	return r | g | b;
}
...
// Output: none
// Must be less than or equal to 128 pixels wide by 160 pixels high
void ST7735_DrawMaskedShape(int16_t x, int16_t y, const bool *mask, uint16_t spriteColor, uint16_t bgColor, int16_t w, int16_t h){
  int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  int i = w*(h - 1);
	int j;
  ...
  for(yl=0; yl<h; yl++){
    for(xl=0; xl<w; xl++){
			j = 195 - ((i / 14) * 14) - (i % 14); // upside down images, oops
			if(mask[j]) {
				pushColor(spriteColor);
			} else {
				pushColor(bgColor);
			}
      i++;
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawChar(_width - 6 - StX*6,_height - 10 -StY*10,ch,StTextColor,ST7735_BLACK, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawChar(_width - 6 - StX*6,_height - 10 -StY*10,'*',ST7735_RED,ST7735_BLACK, 1);
  }
  return;
}
//********ST7735_OutString*****************
